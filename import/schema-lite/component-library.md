...
| promo-banner | cta-default, cta-inverse | **candidate** — token-only surface (no image asset); cta-inverse reuses `color/bg/brand-xstrong` + `color/content/inverse-default` from Highlight Bar (same tokens, first component to implement them); depends on button-group for its single CTA; cta-inverse's CTA falls back from accent to primary button color on non-Evernorth brands until accent buttons are fully spec'd — unverified contrast, flagged for review. Two image-backed variants (cta-default-image, cta-inverse-image) parked, not built |
